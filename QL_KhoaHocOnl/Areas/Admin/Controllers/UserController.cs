using QL_KhoaHocOnl.Controllers;
        ...
        public ActionResult UserCourse(int? ID_USER, string FULLNAME_USER, string PASSWORD, string USERNAME, DateTime BIRTHDAY, string EMAIL_USER, string PHONE_USER, string ROLE_USER, string STATUS_USER)
        {
            using (QL_COURSEEntities1 db = new QL_COURSEEntities1())
            {
                ...
                else
                {
                    ...
                    db.USER_COURSE.Add(new USER_COURSE { FULLNAME_USER = FULLNAME_USER, ROLE_USER = ROLE_USER, PHONE_USER = PHONE_USER, EMAIL_USER = EMAIL_USER, STATUS_USER = STATUS_USER, BIRTHDAY = BIRTHDAY, PASSWORD = acc.mahoa("123"), USERNAME = USERNAME });
                    db.PRO_UPDATE_COUNTLESSONS();
                    db.SaveChanges();
                }
                return View(db.USER_COURSE.ToList());
